    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(RegisterFormModel registerFormModel)
        {
            ...
            if (response.IsSuccess)
            {
                ...
                {
                    // Если token доступа получен, то аутентификация успешна
                ...
                // Если token доступа не получен, можно считать, что пользователь не найден
                TempData["ErrorMessage"] = "Не удалось зарегистрировать пользователя";
                return Page();
            }
            else
            {
